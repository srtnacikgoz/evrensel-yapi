...
editProduct()       // "update" kullan
```

**Endpoint Yapısı (RESTful):**
```
GET     /api/products          → Tüm ürünleri listele
GET     /api/products/:id      → Tekil ürün getir
POST    /api/products          → Yeni ürün oluştur
PUT     /api/products/:id      → Ürünü tamamen güncelle
PATCH   /api/products/:id      → Ürünü kısmi güncelle
DELETE  /api/products/:id      → Ürünü sil
```

**Nested Resource Örneği:**
```
GET     /api/products/:productId/reviews      → Ürünün yorumları
POST    /api/products/:productId/reviews      → Ürüne yorum ekle
DELETE  /api/products/:productId/reviews/:id  → Yorumu sil
```

**Query Parameters:**
```
GET /api/products?page=1&limit=20           → Sayfalama
GET /api/products?sort=price&order=desc     → Sıralama
GET /api/products?category=cikolata         → Filtreleme
GET /api/products?search=bitter             → Arama
```
